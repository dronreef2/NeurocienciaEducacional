...
# Funções internas (helper) - prefixo helper_ (sem underscore no início)
helper_criar_dados_exemplo_ancova <- function(input_file) {
  ...
  dados <- expand.grid(
    id = 1:(n_por_grupo * 5),
    grupo = factor(
      rep(c("ADAP_8", "ADAP_16", "FIXA_8", "FIXA_16", "CTRL"), each = n_por_grupo),
      levels = c("CTRL", "FIXA_8", "FIXA_16", "ADAP_8", "ADAP_16")
    )
  )
  dados$idade <- sample(7:9, nrow(dados), replace = TRUE, prob = c(0.3, 0.5, 0.2))
  dados$ses <- rnorm(nrow(dados), 0, 1)
...
helper_cohens_d_por_grupo <- function(dados, output_dir) {
...
helper_plot_ancova <- function(dados, results, output_dir) {
  for (out in names(results)) {
    ...
    p <- ggplot2::ggplot(emm_df,
      ...
      ggplot2::labs(x = "Grupo", y = "Media marginal (95% IC)",
...
helper_relatorio_ancova <- function(results, correcao, output_dir) {
  relatorio <- paste0(
    "===========================================\n",
    "  RELATORIO - ANCOVA P02\n",
...
#' @return Invisivel
#' @export
ancova_p02 <- function(input_file, output_dir) {
  ...
  if (!file.exists(input_file)) {
    log_msg("Arquivo nao encontrado. Criando dados de exemplo...",
            level = "WARN")
    helper_criar_dados_exemplo_ancova(input_file)
  }
  ...
  for (i in seq_along(outcomes)) {
    ...
    formula <- as.formula(sprintf("%s ~ grupo + %s + idade + sexo", post, pre))
  ...
  helper_cohens_d_por_grupo(dados, output_dir)

  # Plots
  helper_plot_ancova(dados, results, output_dir)

  # Relatorio
  helper_relatorio_ancova(results, correcao, output_dir)

  log_msg("ANCOVA P02 concluida", level = "SUCCESS")
  invisible(results)
}
