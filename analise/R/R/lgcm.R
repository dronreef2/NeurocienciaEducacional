...
helper_criar_dados_exemplo_lgcm <- function(input_file, n_ondas) {
  ...
  dados$idade <- seq(7, length.out = n_ondas)[dados$onda]
  dados$sexo <- rep(sample(c("F", "M"), n, replace = TRUE), each = n_ondas)
  dados$ses <- rep(rnorm(n, 0, 1), each = n_ondas)
  dados$escola <- rep(sample(c("publica", "privada"), n,
                              replace = TRUE, prob = c(0.7, 0.3)),
                       each = n_ondas)
  dados$tempo <- dados$onda - 1
  dados$fe_score <- NA_real_
  ...
  dados <- dados[order(dados$id, dados$onda), ]
  write.csv(dados, input_file, row.names = FALSE)
}


helper_preparar_dados_lgcm <- function(dados, n_ondas) {
  dados_wide <- dados |>
...
helper_comparar_modelos_lgcm <- function(fit_linear, fit_cond, fit_quad) {
  data.frame(
    Modelo = c("Linear", "Condicional", "Quadratico"),
...
helper_relatorio_lgcm <- function(comparacao, output_dir) {
  relatorio <- paste0(
    "===========================================\n",
    "  RELATORIO - LGCM P05\n",
...
#' @param n_ondas Numero de ondas (default 5)
#' @return Lista invisivel com modelos
#' @export
lgcm_p05 <- function(input_file, output_dir, n_ondas = 5) {
  ...
  if (!file.exists(input_file)) {
    log_msg("Arquivo nao encontrado. Criando dados de exemplo...",
            level = "WARN")
    helper_criar_dados_exemplo_lgcm(input_file, n_ondas)
  }

  dados <- read.csv(input_file)
  dados_wide <- helper_preparar_dados_lgcm(dados, n_ondas)
  ...
  # Comparacao
  comparacao <- helper_comparar_modelos_lgcm(fit_linear, fit_cond, fit_quad)
  ...
  # Relatorio
  helper_relatorio_lgcm(comparacao, output_dir)

  log_msg("LGCM P05 concluido", level = "SUCCESS")
...
#'
#' @param dados_wide Wide format data com covariaveis
#' @param n_ondas Numero de ondas
#' @return Objeto lavaan
#' @export
...
plot_trajetorias <- function(dados, output_dir) {
  ...
  p <- ggplot2::ggplot(dados, ggplot2::aes(x = idade, y = fe_score,
    ...
    ggplot2::labs(title = "Trajetorias de desenvolvimento de FE",
